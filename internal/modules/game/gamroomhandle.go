...
func (r *GameRoomData) HandleRoomData(room sockit.Room, server sockit.MeshServer) {
...
// game room
func (r *GameRoomData) handleGameRoommessages(room sockit.Room, server sockit.MeshServer, message *sockit.Message) {
...
func (r *GameRoomData) ClientListNotify(clientsinroom []string, room sockit.Room, server sockit.MeshServer) {
	ret := []*ClientProps{}
...
func (r *GameRoomData) KnowTheClient(clientsinroom []string, room sockit.Room, server sockit.MeshServer) {
	message := &sockit.Message{
...
func (r *GameRoomData) GotRandomRoom(clientsinroom []string, room sockit.Room, server sockit.MeshServer) {
	message := &sockit.Message{
...
func (r *GameRoomData) JoinRoomNotify(clientsinroom []string, room sockit.Room, server sockit.MeshServer) {
	message := &sockit.Message{
	...
	if clientsinroom[2] == room.GetRoomMakerInfo() {
		roommakermessage := &sockit.Message{
...
func (r *GameRoomData) HandleStartGameMessage(target string, room sockit.Room, server sockit.MeshServer) {
	...
	botgreetingsmessage := &sockit.Message{
		Action: "send-message",
		Target: target,
		Sender: "bot-of-the-room",
		MessageBody: map[string]interface{}{
	...
	chattimemessage := &sockit.Message{
	...
	go func() {
		...
		for {
			...
			case <-firsturn:
				...
				r.Letter = "w"
				chattimeendmessage := &sockit.Message{
					Action: "message-by-bot",
					Target: target,
					Sender: "bot-of-the-room",
					MessageBody: map[string]interface{}{
...
func (r *GameRoomData) TurnTheGameTimer(room sockit.Room, server sockit.MeshServer) {
...
func (r *GameRoomData) EndTheGameTimer(room sockit.Room, server sockit.MeshServer) {
